...
HRESULT CMultiTexture::Insert_Texture(const wstring & wstrFilePath, const wstring & wstrStateKey , const int & iCount)
{
...
const TEXINFO * CMultiTexture::Get_TexInfo(const wstring & wstrStateKey , const int & iIndex )
{
	auto& iter = m_mapMultiTexInfo.find(wstrStateKey);
	if (iter == m_mapMultiTexInfo.end())
	{
