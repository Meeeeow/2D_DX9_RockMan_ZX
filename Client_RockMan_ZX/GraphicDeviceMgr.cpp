...
HRESULT CGraphicDeviceMgr::Init_Graphic()
{
	D3DCAPS9 d3dcaps;
...
void CGraphicDeviceMgr::Release_Graphic()
{
	if (m_pLine)
		m_pLine->Release();
	if (m_pFont)
...
void CGraphicDeviceMgr::Render_Begin()
{
	CGraphicDeviceMgr::Get_Instance()->Get_Device()->Clear(0, nullptr,
		D3DCLEAR_TARGET | D3DCLEAR_STENCIL | D3DCLEAR_ZBUFFER, D3DCOLOR_ARGB(255, 0, 255, 0),
		1.f, 0);
	CGraphicDeviceMgr::Get_Instance()->Get_Device()->BeginScene();
	CGraphicDeviceMgr::Get_Instance()->Get_Sprite()->Begin(D3DXSPRITE_ALPHABLEND);
}

void CGraphicDeviceMgr::Render_Stop(HWND hWnd)
{
	CGraphicDeviceMgr::Get_Instance()->Get_Sprite()->End();
	CGraphicDeviceMgr::Get_Instance()->Get_Device()->EndScene();
	CGraphicDeviceMgr::Get_Instance()->Get_Device()->Present(nullptr, nullptr, hWnd, nullptr);
}

CGraphicDeviceMgr::CGraphicDeviceMgr()
{
}
